report 50038 "QBA GSTR-1 File Format"
{
    Caption = 'QBA GSTR-1 File Format';
    ProcessingOnly = true;
    UseRequestPage = true;
    ...
    begin
        ...
        AddTextColumn('TAX amount');
        AddTextColumn('DOMESTIC/EXPORT');
        AddTextColumn('IGST');
        AddTextColumn('CGST');
        AddTextColumn('SGST');
        AddTextColumn('HSN CODE');
        AddTextColumn(CESSAmountTxt);
    end;

    local procedure MakeExcelBodyB2B()
    var
        GSTR1B2BQuery: Query QBA_GSTR1B2BQuery;
        GSTR1EXPQuery: Query GSTR1ExpQuery;//Added by Rohit
    begin
        MakeExcelHeaderB2B();
    ...
    local procedure FillExcelBufferB2B(GSTR1B2BQuery: Query QBA_GSTR1B2BQuery)
    var
    ...
    begin
        ...
        //AddNumberColumn(GSTR1B2BQuery.GST__); // Rohit
        if GSTR1B2BQuery.GST_Jurisdiction_Type = GSTR1B2BQuery.GST_Jurisdiction_Type::Intrastate then
            ...
            AddNumberColumn(GSTR1B2BQuery.GST__);   // Rate
        ...
        if GSTR1B2BQuery.GST_Jurisdiction_Type = GSTR1B2BQuery.GST_Jurisdiction_Type::Intrastate then  // Taxable Value
            AddNumberColumn(Abs(GSTR1B2BQuery.GST_Base_Amount / 2))
        else
            AddNumberColumn(Abs(GSTR1B2BQuery.GST_Base_Amount));
        //
        AddNumberColumn(Abs(GSTR1B2BQuery.GST_Amount)); // TAX Amont
        AddTextColumn(''); // DOMESTIC/EXPORT

        if GSTR1B2BQuery.GST_Jurisdiction_Type = GSTR1B2BQuery.GST_Jurisdiction_Type::Interstate then
            AddNumberColumn(Abs(GSTR1B2BQuery.GST_Amount))
        else
        ...
        AddTextColumn(GSTR1B2BQuery.HSN_SAC_Code); // HSN CODE
        //
    ...
    local procedure FillInvoiceValue(GSTR1B2BQuery: Query QBA_GSTR1B2BQuery)
    var
    ...
    local procedure GetInvoiceType(GSTR1B2BQuery: Query QBA_GSTR1B2BQuery): Text[50]
    begin
    ...
    begin
        ...
        TempExcelBuffer.SetFriendlyFilename(StrSubstNo('GSTR-1 File', CurrentDateTime, UserId));
