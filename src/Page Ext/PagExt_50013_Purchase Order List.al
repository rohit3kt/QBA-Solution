pageextension 50013 PurchaseOrders extends "Purchase Order List"
{
    layout
    {
        addafter("Location Code")
        {
            ...
            field(TotGSTamt; CGSTAmt + IGSTAmt + SGSTAmt)
            {
                Caption = 'GST Amount';
                ApplicationArea = all;
            }
            field(TotalTDSAmt; tdsTotal) //tdsTotal//TotalTDSAmt
            {
                Caption = 'TDS Amount';
                ApplicationArea = all;
            }
            field(NetTotal; Rec.Amount + CGSTAmt + IGSTAmt + SGSTAmt - tdsTotal)
            {
                Caption = 'Net Total';
                ApplicationArea = all;
            }
    ...
    begin
        ...
        tdsTotal:=0;
        purchaseLine7.Reset();
        purchaseLine7.SetRange(purchaseLine7."Document No.", rec."No.");
        if purchaseLine7.FindSet then repeat GetTDSAmount(purchaseLine7.RecordId);
                tdsTotal+=TdsAmt; // TotalTDSAmt;//
            Until purchaseLine7.Next() = 0;
    // Message(Format(tdsTotal));
    end;
    var tdsTotal: Decimal;
    purchaseLine7: Record 39;
    ...
    begin
        ...
        if TaxTransactionValue.FindSet then begin
            repeat TdsAmt:=TaxTransactionValue.Amount;
